...
## forgefirm-docs repository rules

This repository is the documentation site, published at
<https://docs.forgefirm.org/>. **It is the source of truth for every fact
about the machine and the firmware.** A fact lives here, or it does not
exist.
...
zensical serve                    # preview on http://127.0.0.1:8000/
zensical build                    # strict: a broken link fails the build
python scripts/check-style.py
python scripts/check-interfaces.py
```

A pull request runs all three. A red check blocks the merge.

### Rules specific to this repository

- **Write in ASD-STE100 Simplified Technical English, American English, and
  no em dashes.** `check-style.py` enforces the mechanical part.
- **Technical pages describe mechanisms. Usage pages are for a person who
  does not know the jargon**, and they explain the same thing in plain terms
  with a link, never by restating the technical text.
- **Do not write a fact twice.** Link to the page that owns it. Two copies
  drift, and then the reader cannot tell which one is current.
- **Every measured value is written as measured on the bench reference**,
  which `technical/machine/index.md` defines. A measurement is never
  presented as a specification.
- **Every diagram is Mermaid**, in a `<div class="diagram" markdown>`
  wrapper. No ASCII art, and no image of a diagram. Check a new diagram at
  1400 px, 768 px and 390 px, in both color schemes.
- **A moved document is deleted.** No stub and no redirect at the old path.
- Zensical is pinned in `requirements.txt` the way a recipe pin is. Bump it
  on purpose.
- Every push to `main` publishes the site, while the repository variable
  `DEPLOY_PAGES` is set.
