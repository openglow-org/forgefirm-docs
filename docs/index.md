...
!!! danger "In development, not yet released"
    ...
    is installable, and there is no supported way to put this on a machine.
    This site describes the firmware as it is being built and validated on the
    bench. It is here to be read, not followed.

    If you come across a file claiming to be a ForgeFIRM image, it did not
    come from this project.

!!! abstract "What this costs"

    Nothing. It cost nothing yesterday, it costs nothing today, and if anyone
    ever charges you for it, it wasn't this project.

    ForgeFIRM is free in both senses: free as in beer, free as in speech. All
    of it is public. The image, the build layers, the kernel module, the
    control daemon, the grblHAL driver, the cloud client, down to the brackets
    you print. The licenses (MIT and GPL) are the standard ones, and any lawyer
    will tell you they're not a trap. Read it, build it, change it, pass it on.

    No paid tier. No license key, no subscription, no activation, no Pro
    edition, no feature parked behind a paywall. Nothing is held back for a
    rainy day, mostly because there's no plan for a rainy day.
    ...
    page. No waitlist, no pre-order, no backer, no launch, nothing to buy.
    Anyone who spends their evenings deleting sales pitches has earned the
    reflex to suspect one more, so don't take a stranger's word for it. Ten
    minutes with the licenses and the commit log settles it, which beats trust
    on both cost and accuracy. Questions are welcome at
    ...
    that's given away, and that what you take home is their build, not this
    one: a stranger's code, running a laser that has no opinion about what it
    burns. Get it from the source. Same price everywhere, and here you get to
...
- **GRBL mode.** [grblHAL](https://github.com/grblHAL) runs on the machine
  and speaks Grbl 1.1 over TCP port 23, so LightBurn, UGS, and cncjs drive the
  ...
  verification, over-temperature holds, and an operator button press to arm
  the laser for each job.
- **Cloud mode.** The machine presents itself as a stock Glowforge to the
  ...
  camera-referenced homing, until limit-switch homing lands.
...
kill you or others, and voids your warranty. It is not affiliated with or
endorsed by Glowforge. Use it at your own risk.**
