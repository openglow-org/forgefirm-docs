...
| `GET /ext/catalog`, `POST /ext/catalog/refresh`, `POST /ext/catalog/get` (`id`) | The catalog ([the operator's door](extensions.md#the-catalog)): the signed index the extension host keeps, a fresh one fetched and verified, and one listed package fetched into the staging file, held to the size and SHA-256 the index names, and answered with the host's `inspect` as an upload is. Only these reach the network, and only when the operator asks. `502` when a fetch fails, `409` for the host's refusal, for bytes that are not the listed archive, and with no index kept, `404` for an id the index does not list. |
