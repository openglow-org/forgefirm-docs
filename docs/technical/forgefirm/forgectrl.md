...
| `GET /cam/stream`, `GET /cam/snapshot`, `GET /cam/status`, `GET /cam/h264`, the mjpg-streamer aliases | The camera service ([Video pipeline](video-pipeline.md)). `background=1` on a snapshot marks a capture nobody is waiting for: it is refused with 409 while a stream has a client, because a snapshot borrows the mux and stutters a running stream. An operator's own snapshot is unmarked and still wins |
