...
| `forgeext` | `cmake -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS=-Werror && cmake --build build`, then `caps_test`, `manifest_test`, `state_test`, `super_test`, `FWUP=<fwup> python3 -B tests/install_test.py`, and as root `./build/sandbox_test`, `tests/netrules_test.py`, and `tests/run_test.py` | The [extension host](../technical/forgefirm/extensions.md): the closed capability list, the manifest parser and each way of being wrong, `state.json`; `install_test.py` drives the built binary against real archives made with fwup and throwaway keys (the tiers, the product gate, a firmware key, a swapped payload, hostile payloads, the key pin, conflicts, the budget, the integrity check, removal), and nothing a refused archive brought stays behind; `sandbox_test` starts a real child under the launcher (root and cgroup v2) and looks at it from inside and from outside; `super_test` runs the supervisor's rules against fake operations and a fake clock; `netrules_test.py` loads the image's rule file into a network namespace with a peer on a veth pair and drives the allowlist; `run_test.py` runs the host itself in such a namespace against a stand-in for forgectrl's read-only routes and four real packages: the stagger, the confinement, the armed window, quarantine, a second host refused, a killed host's services swept by the next one, safe mode, and a clean end. |
...
| `forgeext` | `build` | The CMake build with `-Werror`, the four C tests, `install_test.py` against fwup 1.16.0 (the version the image carries), and under `sudo` `sandbox_test`, `netrules_test.py`, and `run_test.py` with the rule file from a `forgefirm` checkout. A test that reports it could not run (exit 77) fails the job. |
