...
| `forgeext` | `cmake -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS=-Werror && cmake --build build`, then `caps_test`, `manifest_test`, `state_test`, `super_test`, `holdkeep_test`, `httpreq_test`, `api_test`, `evfeed_test`, `FWUP=<fwup> python3 -B tests/install_test.py`, and as root `./build/sandbox_test`, `tests/netrules_test.py`, and `tests/run_test.py` | The [extension host](../technical/forgefirm/extensions.md): the closed capability list, the manifest parser and each way of being wrong, `state.json`; `install_test.py` drives the built binary against real archives made with fwup and throwaway keys (the tiers, the product gate, a firmware key, a swapped payload, hostile payloads, the key pin, conflicts, the budget, the integrity check, removal), and nothing a refused archive brought stays behind; `sandbox_test` starts a real child under the launcher (root and cgroup v2) and looks at it from inside and from outside; `super_test` runs the supervisor's rules against fake operations and a fake clock; `holdkeep_test` takes what the host says for a package's hold through every case and runs the keeper against a real directory (fresh while the main loop speaks, stale when it goes silent, a clean stop leaving the required files); `httpreq_test` feeds the API socket's request reader what it takes (a byte at a time too) and what it refuses, each with its status; `api_test` runs the broker's judgment over a fake machine (a capability not held, a machine never asked for a package that may not read it, an answer that is no JSON object, the hold's one closed form, and the events poll: its body's form, what comes back after a sequence number, and when the broker is told to make a request wait); `evfeed_test` feeds the server-sent-events reader the lines forgectrl writes (half an event, a keep-alive, a goodbye, a line longer than it holds) and runs the ring a package reads from (where the present is against the beginning of what is held, a place the feed never had, and a reader a whole ring behind told what it lost); `netrules_test.py` loads the image's rule file into a network namespace with a peer on a veth pair and drives the allowlist; `run_test.py` runs the host itself in such a namespace against a stand-in for forgectrl's read-only routes and four real packages: the stagger, the confinement, the API socket used by real sandboxed services (self, the relayed mode, a hold raised and cleared, 403 without the capabilities, sixty requests at once, the socket's owner and mode, a caller that is not the account), the holds (fresh, raised for a required package that keeps ending and never flickering clear, named for forgectrl, gone with extensions off and safe mode), the armed window, quarantine, a second host refused, a killed host's services swept by the next one, safe mode, and a clean end. |
...
| `forgeext` | `build` | The CMake build with `-Werror`, the seven C tests, `install_test.py` against fwup 1.16.0 (the version the image carries), and under `sudo` `sandbox_test`, `netrules_test.py`, and `run_test.py` with the rule file from a `forgefirm` checkout. A test that reports it could not run (exit 77) fails the job. |
...
| `forgefirm-extension-alignment`, `forgefirm-extension-automation` | `package` | forgeext's shared workflow (`.github/workflows/package.yml`, with its `kit` action), on each pull request and push: the package's own tests (automation: its unit tests), the package staged as it ships, linted and packed, and a test under the real extension host as root (automation: `tests/automation_test.py`). On a push to `main` whose manifest version has no release, the release job signs the package and publishes it with its catalog record ([A repository for your package](extensions.md#a-repository-for-your-package)). |
| `forgefirm-extensions-catalog` | `check` | On each pull request and push: the extension host of every `forgeext` revision in `hosts`, built from source (`tools/hosts.sh`), and fwup 1.16.0; `tests/check_test.py` and `tests/publish_test.py`, which hold the check and the publish step to what they promise on throwaway catalogs; then the [catalog's check](extension-listing.md#asking-for-a-listing) (`tools/check.py`). A pull request fetches each new record's archive once, and the workflow's cache keeps it by its SHA-256; a push to `main` fetches no archive. On a push to `main`, after the check, the publish job signs the index and publishes it when it changed ([How the catalog is built](extension-listing.md#how-the-catalog-is-built)). |
