...
| `forgeext` | `cmake -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS=-Werror && cmake --build build`, then `caps_test`, `manifest_test`, `state_test`, `FWUP=<fwup> python3 -B tests/install_test.py`, and `sudo ./build/sandbox_test` | The [extension host](../technical/forgefirm/extensions.md): the closed capability list, the manifest parser and each way of being wrong, `state.json`; `install_test.py` drives the built binary against real archives made with fwup and throwaway keys (the tiers, the product gate, a firmware key, a swapped payload, hostile payloads, the key pin, conflicts, the budget, the integrity check, removal), and nothing a refused archive brought stays behind; `sandbox_test` starts a real child under the launcher (root and cgroup v2) and looks at it from inside and from outside. |
...
| `forgeext` | `build` | The CMake build with `-Werror`, the three C tests, `sandbox_test` under `sudo`, and `install_test.py` against fwup 1.16.0, the version the image carries. A test that reports it could not run (exit 77) fails the job. |
