...
| `forgefirm` | `sandbox-ci` | On a change to the `forgefirm-sandbox` recipe or to the test. One step: `scripts/sandbox-rules-test.py` as root, which loads the [extension sandbox's](../technical/forgefirm/image-and-bsp.md#the-extension-sandbox) rule file into a network namespace of its own and sends at it from real uids. A pool uid is refused on loopback (IPv4 and IPv6, TCP at once, UDP with `EPERM`); root and the uids on either side of the pool are not touched; an allowlist chain opens one destination to one uid; and loading the file again takes the allowlist away. |
