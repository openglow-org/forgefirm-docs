...
| Safety | The CO~2~ warning, the principle (the hardware chain is the safety boundary, and software only adds gates), the gates as the operator sees them, the regulatory and legal notes. |
| Installation | What you need, the A/B slot approach, the procedure, the switch between firmwares, updates, recovery. |
...
- Diagrams are Mermaid in a fenced block; the theme renders them.
