...
|---|---|---|
| [`forgefirm`](https://github.com/openglow-org/forgefirm) | The base of the build and of the release: the `meta-forgefirm` Yocto layer, the kas configuration, the image recipes, the install and release scripts, the acceptance tool (`forgetest/`), the bench actuator firmware (`fixture/`), the bench tools (`scripts/bench/`), and the release artifacts (`releases/`). | MIT for the layer metadata. Each component has its own license. |
...
| [`forgefirm-extension-alignment`](https://github.com/openglow-org/forgefirm-extension-alignment), [`forgefirm-extension-automation`](https://github.com/openglow-org/forgefirm-extension-automation) | OpenGlow's own extension packages, [visual alignment](../usage/extensions/alignment.md) and [notifications and automation](../usage/extensions/automation.md), each in its own repository. They are examples of a package's repository: each builds and tests its package on every push, and signs and publishes each new version as a release, through forgeext's shared workflow ([A repository for your package](extensions.md#a-repository-for-your-package)). They are not part of the image. | MIT |
